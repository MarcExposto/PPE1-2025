...
if [ -n "$MonUrl" ]; then #verifie si il y a qqch dans l'argument avec -n #-ne pour dire si vide faire x
  compte=1
  while read -r line; do
    # Extraire le titre de la page à partir des métadonnées HTML
    Titre=$(curl -s "$line" | ggrep -oP '(?<=<title>)(.*?)(?=</title>)' | head -1) # Extrait le titre de la page
